...
MEASURE_COLS = ["log_prob", "verbal_cat", "verbal_num", "caa_proj", "p_true"]
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Restrict to measures that are both configured AND actually present in the parquet,
    # so reading an older run (e.g. one built before p_true was added) still analyzes cleanly.
    enabled = [
        m for m in MEASURE_COLS
        if m in cfg.confidence_measures and m in df.columns and df[m].notna().any()
    ]
    print(f"[05] analyzing measures: {enabled}")
