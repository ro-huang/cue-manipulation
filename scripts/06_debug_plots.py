...
MEASURES = ["log_prob", "verbal_cat", "verbal_num", "caa_proj"]
NICE = {
...
COND_COLOR = {
    "baseline": "#4f4f4f",
    "random_paragraph": "#1f77b4",   # the yoked control
...
def plot_a_levels(df: pd.DataFrame, out: Path) -> None:
    """A. Bar plot of mean ± 95% CI per condition for accuracy + 4 measures."""
...
def plot_c_paired_scatter(df: pd.DataFrame, out: Path) -> None:
    ...
    fig, axes = plt.subplots(len(cond_list), 4, figsize=(16, 4 * len(cond_list)), sharey=False)
...
def plot_e_baseline_strat(df: pd.DataFrame, items_path: Path, out: Path) -> None:
    ...
    cond_list = [c for c in COND_ORDER if c != "baseline"]
    fig, axes = plt.subplots(len(cond_list), 4, figsize=(16, 4 * len(cond_list)))
    for j, m in enumerate(MEASURES):
        for i, cond in enumerate(cond_list):
            ax = axes[i, j]
...
def plot_m_against_yoked(df: pd.DataFrame, out: Path) -> None:
    """M. The crucial random_paragraph control: levels & paired Δ vs random_paragraph.

    Top row: bar chart of mean per measure, with random_paragraph called out as
    the yoked control instead of the unmodified baseline.
    Bottom row: per-item paired Δ between cue_familiarity & random_paragraph
    (i.e., does cue_familiarity raise confidence ABOVE the noise of any-context?).
    """
    fig, axes = plt.subplots(2, 5, figsize=(18, 8))
    fields = [("correct", "accuracy")] + [(m, NICE[m]) for m in MEASURES]

    # Top: bars per condition
    ...
    # Bottom: paired Δ between cue_fam and random_paragraph (item-matched)
    for ax, (col, name) in zip(axes[1], fields):
        if col == "correct":
            cf = df[df.condition == "cue_familiarity_priming"].drop_duplicates("item_id").set_index("item_id")
            rp = df[df.condition == "random_paragraph"].drop_duplicates("item_id").set_index("item_id")
            mrg = cf.join(rp, lsuffix="_cf", rsuffix="_rp", how="inner")
            d = (mrg["correct_cf"].astype(int) - mrg["correct_rp"].astype(int)).dropna()
        else:
            cf = df[df.condition == "cue_familiarity_priming"].drop_duplicates("item_id").set_index("item_id")
            rp = df[df.condition == "random_paragraph"].drop_duplicates("item_id").set_index("item_id")
            mrg = cf.join(rp, lsuffix="_cf", rsuffix="_rp", how="inner")
            d = (mrg[f"{col}_cf"] - mrg[f"{col}_rp"]).dropna()
        m = float(d.mean())
        lo, hi = _ci(d.values)
        t = st.ttest_1samp(d, 0)
        color = COND_COLOR["cue_familiarity_priming"]
        ax.bar(["cue_fam − random_para"], [m], yerr=[[m - lo], [hi - m]],
               color=color, edgecolor="black", capsize=4)
        ax.axhline(0, color="grey", linewidth=0.5)
        ax.set_title(f"Δ {name}\nt={t.statistic:.1f}, p={t.pvalue:.1g}, n={len(d)}", fontsize=9)
        ax.grid(axis="y", alpha=0.3)
    fig.suptitle(
        "M. Random_paragraph yoked control. "
        "Top: levels per condition. Bottom: cue_familiarity − random_paragraph (paired, item-matched). "
        "Bars > 0 = cue familiarity contributes ABOVE generic-context effect.",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(out / "M_vs_random_paragraph.png", dpi=130)
    plt.close(fig)
