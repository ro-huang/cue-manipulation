...
def yoke_whitespace_primes(prime_sets: list[PrimeSet]) -> None:
    """Fill `whitespace` primes with N spaces, where N = character length of the
    item's `random_paragraph` prime. If random_paragraph isn't present, fall back
    to longest other prime. This is the strictest "content-free" control.
    """
    for ps in prime_sets:
        if "whitespace" not in ps.primes:
            continue
        target_chars = 0
        if "random_paragraph" in ps.primes and ps.primes["random_paragraph"].text:
            target_chars = len(ps.primes["random_paragraph"].text)
        else:
            for cond, prime in ps.primes.items():
                if cond == "whitespace" or prime.text is None:
                    continue
                target_chars = max(target_chars, len(prime.text))
        if target_chars == 0:
            target_chars = 300
        ps.primes["whitespace"] = Prime(
            condition="whitespace", text=" " * target_chars,
        )
